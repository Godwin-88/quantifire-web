import Link from 'next/link'

const CHANNELS = [
  { label: 'YouTube', href: 'https://youtube.com/@quantifaya', icon: '▶' },
  { label: 'TikTok', href: 'https://tiktok.com/@quantifaya', icon: '♪' },
  { label: 'Instagram', href: 'https://instagram.com/quantifir3', icon: '◎' },
  { label: 'LinkedIn', href: 'https://linkedin.com/company/quantifaya', icon: 'in' },
  { label: 'Twitter/X', href: 'https://x.com/quantifaya', icon: '𝕏' },
]

const FOOTER_LINKS = {
  Services: [
    { label: 'Quantitative Research', href: '/services/quantitative-research' },
    { label: 'Web Development', href: '/services/web-development' },
    { label: 'Process Automation', href: '/services/process-automation' },
    { label: 'Data Engineering', href: '/services/data-engineering' },
    { label: 'Data Analytics', href: '/services/data-analytics' },
  ],
  Work: [
    { label: 'Selected Work', href: '/portfolio' },
    { label: 'ClinicalMatch AI', href: '/portfolio/clinicalmatch' },
    { label: 'HazardGraph', href: '/portfolio/hazardgraph' },
    { label: 'GraphAlpha', href: '/portfolio/graphalpha' },
  ],
  Research: [
    { label: 'Notebooks', href: '/marketplace' },
    { label: 'Blog', href: '/blog' },
    { label: 'Classical Quant Finance', href: '/blog' },
    { label: 'DeFi Mechanics', href: '/blog' },
  ],
  Company: [
    { label: 'About', href: '/about' },
    { label: 'Contact', href: '/contact' },
    { label: 'Privacy Policy', href: '/privacy' },
    { label: 'Terms of Service', href: '/terms' },
  ],
}

export function Footer() {
  return (
    <footer className="border-t border-slate-800 bg-qf-navy/40">
      <div className="section py-12">
        {/* Top row */}
        <div className="grid grid-cols-2 gap-8 md:grid-cols-4 lg:grid-cols-5">
          {/* Brand */}
          <div className="col-span-2 md:col-span-1 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2">
              <img
                src="/logo.jpg"
                alt="Quantifaya"
                className="h-9 w-9 rounded-lg object-cover ring-1 ring-slate-700/60"
              />
              <span className="text-lg font-bold font-heading">
                <span className="gradient-text">Quanti</span>
                <span className="text-brand-primary">Faya</span>
              </span>
            </Link>
            <p className="mt-3 text-xs text-slate-500 leading-relaxed">
              Quant AI systems for institutions.
              Research for practitioners.
              <br />Nairobi, Kenya
            </p>
            {/* Social channels */}
            <div className="mt-4 flex flex-wrap gap-2">
              {CHANNELS.map((ch) => (
                <a
                  key={ch.label}
                  href={ch.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  title={ch.label}
                  className="flex h-7 w-7 items-center justify-center rounded-md border border-slate-700 text-xs text-slate-400 hover:border-slate-500 hover:text-slate-200 transition-colors"
                >
                  {ch.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Links */}
          {Object.entries(FOOTER_LINKS).map(([section, links]) => (
            <div key={section}>
              <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3">
                {section}
              </h4>
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link.href + link.label}>
                    <Link
                      href={link.href}
                      className="text-sm text-slate-500 hover:text-slate-200 transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        {/* Bottom bar */}
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-slate-800 pt-6 sm:flex-row">
          <p className="text-xs text-slate-600">
            © {new Date().getFullYear()} Quantifaya. All rights reserved.
          </p>
          <p className="text-xs text-slate-600">
            Digital agency · Quant research platform.
          </p>
        </div>
      </div>
    </footer>
  )
}