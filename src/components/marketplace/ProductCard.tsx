...
const CATEGORY_BADGES: Record<string, { label: string; cls: string }> = {
  tradfi:    { label: 'TradFi',    cls: 'bg-blue-500/15 text-blue-400 border-blue-500/30' },
...
export function ProductCard({ product, featured = false }: Props) {
  const cat = CATEGORY_BADGES[product.category]
  const status = STATUS_BADGES[product.status]
      ...
      */}
      ...
      <div className="mt-5 flex items-center justify-between border-t border-slate-800 pt-4">
        <span className="text-xs text-slate-400">
          {product.status === 'live' ? 'Live now' : 'Launching soon'}
        </span>
        <span className="text-xs text-slate-500 group-hover:text-[var(--accent)] transition-colors">
          View Details →
        </span>
